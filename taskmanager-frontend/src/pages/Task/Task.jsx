...
const Task = () => {
  ...
  const createTaskModalState = useSelector(
    (state) => state.createTaskModalObj.isCreateTaskModalSliceModalOpen
  );
  ...
  const handleDrop = async (event, destinationStatus) => {
    ...
    if (taskId) {
      try {
        const response = await updateTaskStatusFunc(
          taskId,
          destinationStatus,
          config
        );
  ...
  return (
    <>
      {createTaskModalState && (
        <CreateTaskModal fetchTasks={fetchTasks} action={action} />
      )}
      <Navbar />
      <div className="page-container">
        ...
        <div className="page-content">
          <div className="add-task-box">
            <div>
              <p className="add-task-title">Manage your tasks easily</p>
            </div>
            <div>
              <button
  ...
  return (
    ...
    >
      ...
      {filteredTasks.map((task) => (
        ...
        >
          <div
            style={{ color: "#ffffff", fontSize: "20px", margin: "4px 0 8px" }}
          >
            {task.name}{" "}
          </div>
          <span
